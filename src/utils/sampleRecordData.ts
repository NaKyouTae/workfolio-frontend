...
export const createSampleRecords = (recordGroups: object[]) => {
    ...
    months.forEach((month) => {
        ...
        for (let i = 0; i < 8; i++) {
            ...
            const titles = [
                '주간 미팅', '프로젝트 리뷰', '코드 리뷰', '기획 회의',
                '디자인 리뷰', 'API 문서화', '데이터베이스 최적화', '모니터링 설정',
                '로깅 시스템 구축', '성능 벤치마크', '문서화', '사용자 인터뷰',
                '데이터 분석', '보안 점검', 'CI/CD 파이프라인', '테스트'
            ];
            ...
            records.push({
                ...
                endedAt: date.hour(hour + duration).minute(minute).second(0).valueOf(),
        ...
        for (let i = 0; i < 4; i++) {
            ...
            const titles = [
                '클라이언트 미팅', '출장', '사용자 교육', '테스트',
                '마이그레이션 계획', '로드 밸런싱', '성능 최적화', '보안 점검',
                '인프라 구축', '사용자 리서치', '프로토타입 개발', '배포 준비'
            ];
            ...
            records.push({
                ...
                title: titles[Math.floor(Math.random() * titles.length)],
                type: 'DAY',
                description: `${titles[Math.floor(Math.random() * titles.length)]} 관련 업무를 하루 종일 진행했습니다.`,
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const titles = [
                '코드 리뷰', '사용자 인터뷰', '성능 벤치마크', '인프라 구축',
                '재해 복구', '데이터 마이그레이션', '기획 회의', '사용자 테스트',
                '최종 테스트', '런칭 준비', '분산 추적', '바운디드 컨텍스트'
            ];
            ...
            records.push({
                ...
                title: titles[Math.floor(Math.random() * titles.length)],
                type: 'MULTI_DAY',
                description: `${titles[Math.floor(Math.random() * titles.length)]} 관련 업무를 여러 일에 걸쳐 진행했습니다.`,
