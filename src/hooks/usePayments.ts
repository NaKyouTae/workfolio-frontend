...
import { Payment, Payment_PaymentStatus, Payment_PaymentMethod } from '@/generated/common';
...
export function getPaymentStatusLabel(status: Payment_PaymentStatus | string): string {
    const statusValue = typeof status === 'string' ? status : Payment_PaymentStatus[status];
    switch (statusValue) {
        case 'PENDING':
...
export function getPaymentMethodLabel(method: Payment_PaymentMethod | string): string {
    const methodValue = typeof method === 'string' ? method : Payment_PaymentMethod[method];
    switch (methodValue) {
        case 'CARD':
