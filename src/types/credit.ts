...
export function getTxTypeLabel(txType: CreditTxType | string): string {
    ...
    switch (type) {
        case 'CHARGE':
        case CreditTxType.CHARGE:
            return '충전';
        case 'BONUS':
        case CreditTxType.BONUS:
            return '보너스';
        case 'USE':
        case CreditTxType.USE:
            return '사용';
        case 'REFUND':
        case CreditTxType.REFUND:
            return '환불';
        case 'ADMIN_ADD':
        case CreditTxType.ADMIN_ADD:
            return '관리자 지급';
        case 'ADMIN_DEDUCT':
        case CreditTxType.ADMIN_DEDUCT:
            return '관리자 차감';
        default:
            return '알 수 없음';
    }
...
export function isCreditAddition(txType: CreditTxType | string): boolean {
    ...
    return type === 'CHARGE' || type === 'BONUS' || type === 'REFUND' || type === 'ADMIN_ADD' ||
           txType === CreditTxType.CHARGE || txType === CreditTxType.BONUS ||
           txType === CreditTxType.REFUND || txType === CreditTxType.ADMIN_ADD;
}
