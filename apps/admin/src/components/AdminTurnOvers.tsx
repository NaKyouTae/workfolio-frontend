...
import { useAdminTurnOvers } from "@/hooks/useAdminTurnOvers";
...
export default function AdminTurnOvers() {
    const router = useRouter();
    const { turnOvers, totalElements, totalPages, currentPage, loading, error, fetchTurnOvers } = useAdminTurnOvers();
    const { selectedWorker, selectWorker, clearWorker } = useSelectedWorker();
    const [pageSize, setPageSize] = useState(DEFAULT_PAGE_SIZE);

    useEffect(() => {
        if (selectedWorker) {
            fetchTurnOvers(selectedWorker.id, 0, pageSize);
        }
    }, [fetchTurnOvers, pageSize, selectedWorker]);

    const handlePageChange = (page: number) => {
        if (selectedWorker) {
            fetchTurnOvers(selectedWorker.id, page - 1, pageSize);
        }
    };
    ...
    const columns: TableColumn<TurnOver>[] = [
        {
            key: "name",
            title: "프로젝트명",
            width: "200px",
        ...
        },
        {
            ...
            render: (t) => (
                <span style={{ color: "#121212" }}>
                    {formatDate(t.startedAt)} ~ {formatDate(t.endedAt)}
                </span>
            ),
        },
        {
            ...
            title: "생성일",
            width: "160px",
            render: (t) => <span style={{ color: "#121212" }}>{formatDateTime(t.createdAt)}</span>,
        },
        {
            key: "updatedAt",
            title: "수정일",
            width: "160px",
            render: (t) => <span style={{ color: "#121212" }}>{formatDateTime(t.updatedAt)}</span>,
        },
    ];
                ...
                {selectedWorker && (
                    <div className="cont-box">
                        <div className="cont-tit">
                            <div>
                                <h3>이직 내역 ({totalElements.toLocaleString()}건)</h3>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap", justifyContent: "flex-end" }}>
                                ...
                                <button
                                    onClick={() => selectedWorker && fetchTurnOvers(selectedWorker.id, currentPage, pageSize)}
                                    disabled={loading}
                                    title="새로고침"
                        ...
                        {!loading && (
                            <>
                                <TableView
                                    columns={columns}
                                    data={turnOvers}
                                    onRowClick={(turnOver) => router.push(`/dashboard/turnovers/${turnOver.id}`)}
                                    getRowKey={(t) => t.id}
                                    emptyMessage="이직 내역이 없습니다."
                                    isLoading={loading}
                                />
                                {totalPages > 1 && (
                                    <Pagination
