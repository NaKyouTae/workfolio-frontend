...
import { Worker_Gender, Worker_WorkerStatus } from "@workfolio/shared/generated/common";
import DateUtil from "@workfolio/shared/utils/DateUtil";
import { normalizeEnumValue, formatPhoneNumber } from "@workfolio/shared/utils/commonUtils";
import Dropdown from "@workfolio/shared/ui/Dropdown";
import DatePicker from "@workfolio/shared/ui/DatePicker";
import { DateTime } from "luxon";
import { WorkerUpdateRequest, WorkerCheckNickNameResponse } from "@workfolio/shared/generated/worker";
...
const ProfileManagement: React.FC = () => {
    ...
    const handleCheckNickName = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `/api/workers/check/${encodeURIComponent(nickName.trim())}`,
                {
                    method: "GET",
                }
            );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const requestBody: WorkerUpdateRequest = {
                id: user?.id ?? "",
                status: user?.status ?? Worker_WorkerStatus.ACTIVE,
                nickName: nickName,
    ...
    return (
        <>
                        ...
                        <ul className="setting-list grid-2">
                            ...
                            <li>
                                ...
                                <DatePicker
                                    value={birthDate}
                                    onChange={(date) =>
                                        setBirthDate(DateTime.fromISO(date).toMillis())
                                    }
                                    required={false}
                                />
                            </li>
                            <li>
                                ...
                                <Dropdown
                                    ...
                                    setValue={(value) =>
                                        setGender(normalizeEnumValue(value, Worker_Gender))
                                    }
                                />
                            </li>
                        </ul>
            ...
            <FloatingNavigation
                navigationItems={[{ id: "profile", label: "프로필 관리" }]}
